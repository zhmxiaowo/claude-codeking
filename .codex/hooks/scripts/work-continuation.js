#!/usr/bin/env node
// Stop hook: /work 循环中存在 pending 任务时强制 Codex 继续
// 注入消息保持极简，避免污染对话历史
...
const stopPath = path.join(cwd, '.codex', '.work-stop');
const pausePath = path.join(cwd, '.codex', '.work-pause');
...
try {
  ...
  // 极简注入：只一个标记，让 Codex 知道继续 work 循环
