...
6. 若存在 `.claude/.work-stop`，读取停止原因，删除该文件，然后继续恢复。
7. 运行 `node .claude/hooks/scripts/next-task.js` 获取当前任务；脚本会跳过 `completed`、`blocked`、`cancelled` 和依赖未完成的 pending 任务。
...
- 按 `CLAUDE.md`、Startup 已加载项目规则和当前 task 实现。
...
- `milestone`：启动 qa-verifier 验证完整模块或连续任务主路径。
- `release`：启动 qa-verifier 做完整回归。

当且仅当满足以下之一时启动 qa-verifier：
...
1. 运行 `node .claude/hooks/scripts/next-task.js`。
