...
- **必须包含「验证与验收策略」章节**：说明 local / slice / milestone / release 四层验证
- 写入项目根目录 `spec.md`

### 5.2 生成 task.json
- 将 spec.md 中的功能拆解为原子任务
- 每个任务包含：id, title, description, status("pending"), dependencies, complexity, doneWhen, verificationLevel, files, notes, origin("init")
...
- `verificationLevel` 只分四档：
	...
	- `release`：完整回归
- 不要为每个微任务都分配重型 QA；只在验收范围需要时使用 `milestone` 或 `release`
- 写入项目根目录 `task.json`
...
执行 /learn 逻辑，将本次 init 对话中的隐性知识提炼到项目根目录 `experience.md`（与 AGENTS.md 同级）：
